...
def cbc_decrypt(C, K, IV):
    ...
    for i in range(nblocks):
        block = C[(i * 16):((i+1)* 16)]
        decrypted = decrypt(block, K)
        xor = XOR(decrypted, prev_block)
        plaintext.extend(xor)
        prev_block = block
    return plaintext
...
def main():
    ...
    IV = bytes_to_bits(read_file(ivf))
    P = cbc_decrypt(C, K, IV)
    write_file(output, bits_to_bytes(P))
